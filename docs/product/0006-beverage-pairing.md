...
When a drinks or wine section is detected in the scanned menu, the app offers pairing suggestions for individual dishes. The user taps any food item to see which beverages from the restaurant's list pair well with it, along with a brief reason.
...
- The drinks section flag (`hasBeverageMenu: boolean`) is part of the digitization response schema in `packages/lib`.
- Pairing results are cached in local session state to avoid repeat AI calls for the same dish within the same session.
